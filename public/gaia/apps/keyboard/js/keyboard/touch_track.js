...
(function(exports) {
  ...
  var TouchTrack = function(app) {
    ...
    this.testFn = function(a,b,c){
        debugger;
    }
  };

  TouchTrack.prototype.start = function(){
    this.app.console.log('TouchTrack.start()');
    if (this._started) {
    ...
    window.addEventListener('tt_test', this.testFn, false);

  }

  TouchTrack.prototype.stop = function(){
    this.app.console.log('TouchTrack.stop()');
    if (!this._started) {
      throw new Error('TouchTrack: ' +
    ...
    window.removeEventListener('tt_test', this.testFn, false);
  }

  TouchTrack.prototype.handleEvent = function(evt, ignoreSave) {
    switch(evt.type){
      case 'message':
        console.log('message in TouchTrack eventhandler');
        break;
      default:
        if(!ignoreSave){
